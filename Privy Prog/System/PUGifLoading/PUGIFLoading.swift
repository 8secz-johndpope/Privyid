...
public class PUGIFLoading
{
    ...
    public func show(_ loadingText : String?, gifimagename: String?, bg: Bool = true)
    {
        if bg{
            hide()

            if #available(iOS 13.0, *) {
                let overlayvw = UIApplication.shared.connectedScenes
                ...
                let overlay = UIView(frame: overlayvw?.frame ?? CGRect(x: 0, y: 0, width: 100, height: 100))
                overlay.center = overlayvw?.center ?? CGPoint(x: 0, y: 0)
                overlay.alpha = 0
                ...
                imageView.frame = CGRect(x: 20.0, y: 50.0, width: 50, height: 50)
            ...
            if #available(iOS 13.0, *) {
                ...
                imageView.frame = CGRect(x: 20.0, y: 50.0, width: 50, height: 50)
                imageView.center = overlayvw?.center ?? CGPoint(x: 0, y: 0);
                overlayvw?.addSubview(imageView)

                UIView.beginAnimations(nil, context: nil)
                UIView.setAnimationDuration(0.5)
                UIView.commitAnimations()
                recentOverlay = overlayvw
                ...
                imageView = UIImageView(image: jeremyGif)
                imageView.frame = CGRect(x: 20.0, y: 50.0, width: 50, height: 50)
                imageView.center = overlayvw.center;
                overlayvw.addSubview(imageView)
...
//        let animation = UIImage.animatedImage(with: frames, duration: Double(duration) / 1000.0)
        ...
        let animation = UIImage.animatedImage(with: frames, duration: 0 / 1000.0)
